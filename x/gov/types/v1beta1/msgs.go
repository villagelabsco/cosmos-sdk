...
func (m MsgSubmitProposal) ValidateBasic() error {
	...
	if !m.InitialDeposit.IsValid() {
		return errors.Wrap(sdkerrors.ErrInvalidCoins, m.InitialDeposit.String())
	}
	...
	if content == nil {
		return errors.Wrap(types.ErrInvalidProposalContent, "missing content")
	}
	if !IsValidProposalType(content.ProposalType()) {
		return errors.Wrap(types.ErrInvalidProposalType, content.ProposalType())
	}
...
func (msg MsgVote) ValidateBasic() error {
	...
	if !ValidVoteOption(msg.Option) {
		return errors.Wrap(types.ErrInvalidVote, msg.Option.String())
	}
...
func (msg MsgVoteWeighted) ValidateBasic() error {
	...
	if len(msg.Options) == 0 {
		return errors.Wrap(sdkerrors.ErrInvalidRequest, WeightedVoteOptions(msg.Options).String())
	}
	...
	for _, option := range msg.Options {
		...
		if usedOptions[option.Option] {
			return errors.Wrap(types.ErrInvalidVote, "Duplicated vote option")
		}
		usedOptions[option.Option] = true
	}

	if totalWeight.GT(math.LegacyNewDec(1)) {
		return errors.Wrap(types.ErrInvalidVote, "Total weight overflow 1.00")
	}

	if totalWeight.LT(math.LegacyNewDec(1)) {
		return errors.Wrap(types.ErrInvalidVote, "Total weight lower than 1.00")
	}
