...
func (c configurator) RegisterMigration(moduleName string, fromVersion uint64, handler MigrationHandler) error {
	if fromVersion == 0 {
		return errors.Wrap(sdkerrors.ErrInvalidVersion, "module migration versions should start at 1")
	}
	...
	if c.migrations[moduleName][fromVersion] != nil {
		return errors.Wrapf(sdkerrors.ErrLogic, "another migration for module %s and version %d already exists", moduleName, fromVersion)
	}
...
func (c configurator) runModuleMigrations(ctx sdk.Context, moduleName string, fromVersion, toVersion uint64) error {
	...
	if !found {
		return errors.Wrapf(sdkerrors.ErrNotFound, "no migrations found for module %s", moduleName)
	}
	...
	for i := fromVersion; i < toVersion; i++ {
		...
		if !found {
			return errors.Wrapf(sdkerrors.ErrNotFound, "no migration found for module %s from version %d to version %d", moduleName, i, i+1)
		}
